...
data_output_location = data_source + '/esc10_2_all_mixes_8sec'
    ...
    for B_class in range(A_class + 1, num_class):
        ...
        for A_file in range(len(A_class_files)):
            ...
            for B_file in range(len(B_class_files)):
                ...
                """
                8-sec: Overlapped Audio
                Each audio is overlapped on a 2-sec portion of each audio. The rest 3-sec are not overlapped.
                To do that, each audio duration is enlarged to 8 seconds by adding 3-sec blank audio.
                The 3-sec blank part is placed in front for one audio.
                The other audio connects the blank part at back.
                """
                blank_3sec = np.zeros(sr*3)
                np_A_data = np.concatenate((np_A_data, blank_3sec), axis=0)
                np_B_data = np.concatenate((blank_3sec, np_B_data), axis=0)
