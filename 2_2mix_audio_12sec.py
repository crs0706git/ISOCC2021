...
data_output_location = data_source + '/esc10_2_all_mixes_12sec'
    ...
    for B_class in range(A_class + 1, num_class):
        ...
        for A_file in range(len(A_class_files)):
            ...
            for B_file in range(len(B_class_files)):
                ...
                """
                12-sec: Separated Audio
                A silence part exists between each audio.
                Each audio duration is enlarged to 12 seconds by concatenating 7-sec blank audio.
                Then, the audio streams are mixed
                """
                blank_7sec = np.zeros(sr * 7)
                np_A_data = np.concatenate((np_A_data, blank_7sec), axis=0)
                np_B_data = np.concatenate((blank_7sec, np_B_data), axis=0)

                rate = 0.75
                np_mix = np_A_data * rate + np_B_data * rate
        ...
        print("Time for mixing %s and %s (%d/%d):" % (
        the_classes[A_class], the_classes[B_class], cls_count, num_class_combi), end=" ")
